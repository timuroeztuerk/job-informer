...
class DescriptionStore:
    ...
    def enqueue(self, job_ids: list[str], *, refresh=False) -> int:
        ...
        with self.connect() as conn:
            ...
            for job_id in dict.fromkeys(job_ids):
                if not refresh and conn.execute("""SELECT 1 FROM description_sources s
                    JOIN description_extractions e USING(source_id)
                    WHERE s.job_id=? AND e.extractor=? LIMIT 1""", (job_id, EXTRACTOR)).fetchone():
                    continue
                added += conn.execute("""INSERT OR IGNORE INTO description_fetches
                    (job_id, source_url, requested_at, status) VALUES (?, ?, ?, 'queued')""",
                    (job_id, source_url(job_id), utc_now_iso())).rowcount
        return added

    def next_candidates(self, limit=10) -> list[str]:
        with self.connect() as conn:
            rows = conn.execute("""SELECT j.job_id FROM jobs j
                WHERE LOWER(j.source)='linkedin' AND (j.archived_at IS NULL OR j.is_favorite=1)
                  AND j.job_id GLOB 'linkedin:[0-9]*'
    ...
    def latest_source(self, job_id) -> dict | None:
        with self.connect() as conn:
            row = conn.execute("""SELECT s.* FROM description_fetches f
                JOIN description_sources s USING(source_id) WHERE f.job_id=?
    ...
    def job_description(self, job_id) -> dict:
        self.require_job(job_id)
        with self.connect() as conn:
            attempts = [dict(row) for row in conn.execute("""SELECT fetch_id, requested_at, started_at,
                finished_at, status, http_status, error_kind, error_message, source_id
                FROM description_fetches WHERE job_id=? ORDER BY fetch_id DESC LIMIT 8""", (job_id,))]
            row = conn.execute("""SELECT s.source_id, s.source_url, s.fetched_at,
                e.extraction_id, e.extractor, e.extractor_version, e.schema_version, e.extracted_at,
                e.content_sha256, e.data_json
                FROM description_fetches f JOIN description_sources s USING(source_id)
                JOIN description_extractions e USING(source_id)
                WHERE f.job_id=? AND e.extractor=?
                ORDER BY f.fetch_id DESC, e.extraction_id DESC LIMIT 1""", (job_id, EXTRACTOR)).fetchone()
            sources = conn.execute("SELECT COUNT(*) FROM description_sources WHERE job_id=?", (job_id,)).fetchone()[0]
        ...
        return {"job_id": job_id, "saved": saved, "attempts": attempts, "source_versions": sources,
                "reparse_available": bool(sources), "parser_version": EXTRACTOR_VERSION}
    ...
    def queue_status(self) -> dict:
        with self.connect() as conn:
            ...
            saved = conn.execute("SELECT COUNT(DISTINCT s.job_id) FROM description_sources s JOIN description_extractions e USING(source_id) WHERE e.extractor=?", (EXTRACTOR,)).fetchone()[0]
            failed = conn.execute("""SELECT COUNT(*) FROM description_fetches f WHERE status IN ('failed','interrupted')
                AND NOT EXISTS (SELECT 1 FROM description_fetches newer WHERE newer.job_id=f.job_id AND newer.fetch_id>f.fetch_id)""").fetchone()[0]
