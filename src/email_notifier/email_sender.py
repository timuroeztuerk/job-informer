...
class EmailSender:
    ...
    def send_with_attachment(self, subject: str, body: str, attachment_path: str) -> bool:
        ...
        try:
            ...
            # Create modern HTML version of the body
            html_body = self._create_attachment_email_html(body, attachment_path)
    ...
    def _create_attachment_email_html(self, body: str, attachment_path: str) -> str:
        """Create HTML template for email with attachment"""
        from datetime import datetime
        import os
        
        filename = os.path.basename(attachment_path) if os.path.exists(attachment_path) else "File not found"
        file_size = ""
        if os.path.exists(attachment_path):
        ...
        <head>
          <meta charset="utf-8" />
          <meta name="viewport" content="width=device-width,initial-scale=1" />
          <title>Job Informer Email with Attachment</title>
          <style>
            :root {{
              --bg: #0b0c10;
              --card: #121318;
            ...
            * {{ box-sizing: border-box; }}
            ...
            .header {{
              background: radial-gradient(1200px 500px at 10% -10%, rgba(106,169,255,.20), transparent 60%),
                          radial-gradient(900px 400px at 100% 0%, rgba(0,209,178,.12), transparent 60%),
                          linear-gradient(180deg, var(--card), var(--card-2));
              border: 1px solid var(--border);
              border-radius: var(--radius);
              padding: 28px 28px 24px;
            ...
            .panel .panel-header {{
              ...
              padding: 16px 20px;
              border-bottom: 1px solid var(--border);
              background: linear-gradient(180deg, rgba(106,169,255,.10), transparent 70%);
            }}
            .chip {{
              display: inline-flex; align-items: center; gap: 8px;
              padding: 6px 10px;
        ...
        <body>
          <div class="container">
            ...
            <section class="header">
              <h1>📎 Job Informer Email</h1>
              <p class="subtle">Message with attachment</p>
            </section>

            <section class="panel">
              <div class="panel-header">
                <span class="chip">💬 Message</span>
                <span class="muted">Email content</span>
              </div>
              <div class="panel-body">
                ...
                <div class="attachment-info">
                  ...
                  <div class="attachment-details">
                    <h4>{filename}</h4>
                    <p>{file_size if file_size else 'Attachment included'}</p>
                  </div>
                </div>
              </div>
            </section>
            ...
            <div class="footer">
              Generated by Job Informer · <span class="muted">Sent on {datetime.now().strftime('%B %d, %Y at %I:%M %p')}</span> 📧
            </div>
    ...
    def send_market_report(self, market_report: str, subject: str, total_jobs: int) -> bool:
        """Send AI-generated market analysis report"""
        try:
            # Create HTML content for market report
            html_content = self._create_market_report_html(market_report, total_jobs)
    ...
    def _create_market_report_html(self, market_report: str, total_jobs: int) -> str:
        """Create HTML template for market analysis report"""
        from datetime import datetime
        import re
        
        # Convert markdown-style formatting to HTML
        formatted_report = self._convert_markdown_to_html(market_report)
        ...
        <head>
          <meta charset="utf-8" />
          <meta name="viewport" content="width=device-width,initial-scale=1" />
          <title>AI Job Market Analysis — {datetime.now().strftime('%b %d, %Y')}</title>
          <style>
            :root {{
              --bg: #0b0c10;
              --card: #121318;
            ...
            * {{ box-sizing: border-box; }}
            ...
            .prose h2 {{ font-size: 20px; border-bottom: 1px solid var(--border); padding-bottom: 6px; color: var(--accent); }}
            .prose h3 {{ font-size: 18px; color: var(--accent-2); }}
        ...
        <body>
          <div class="container">
            ...
            <section class="header">
              <h1>🤖 AI Job Market Analysis</h1>
              <p class="subtle">Generated on {datetime.now().strftime('%B %d, %Y at %I:%M %p')}</p>
              <div class="kpis">
                <div class="kpi">
                  <div class="label">Total Jobs</div>
                ...
                <div class="kpi">
                  ...
                  <div class="value">Google Gemini</div>
                </div>
                <div class="kpi">
                  <div class="label">Report Type</div>
                  <div class="value">Market Snapshot</div>
                </div>
                <div class="kpi">
            ...
            <section class="panel">
              <div class="panel-header">
                <span class="chip">📊 Analysis Overview</span>
                <span class="muted">Concise, data-driven insights</span>
              </div>
            ...
            <div class="footer">
              Generated by Job Informer · <span class="muted">Insights based on scraped job data; intended for guidance.</span> 📈
            </div>
