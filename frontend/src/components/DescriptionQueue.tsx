...
const DescriptionQueue: React.FC<Props> = ({ refreshToken, onChanged }) => {
  ...
  const act = async (action: "next" | "retry" | "pause" | "resume") => {
    setBusy(true);
    setError(null);
    try {
      const value = action === "next" || action === "retry" ? await queueDescriptions(action) : await controlDescriptionQueue(action);
      setState(value);
  ...
  return <section className="panel description-queue" aria-label="Description retrieval">
    <div className="description-queue-bar"><div><p className="label">Descriptions</p><p className="muted small">{state ? `${state.saved_jobs} saved · ${state.queued} queued${state.fetching ? " · Fetching one" : ""}${state.failed_jobs ? ` · ${state.failed_jobs} need attention` : ""}` : "Loading retrieval status…"}</p></div>
      <div className="description-queue-actions">
        <button type="button" className="ghost sm" disabled={busy || !state} onClick={() => void act("next")}>Fetch next 10</button>
    ...
    <p className="muted small">Manual batches use favorites first, then active jobs without an earlier attempt. Saved descriptions are reused. Retry failed adds up to 10 jobs.</p>
